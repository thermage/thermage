...
test('test heading size < 1', function (): void {
    ...
    expect(str_replace([PHP_EOL, " "], "", $value))->toBe("\e[0m╔══════════════════╗\e[0m\e[0m║\e[1mRAD\e[22m║\e[0m╚══════════════════╝\e[0m");
});
...
test('test heading size 3', function (): void {
    ...
    expect(str_replace([PHP_EOL, " "], "", $value))->toBe("\e[0m┌──────────────────┐\e[0m\e[0m│RAD│\e[0m└──────────────────┘\e[0m");
});

test('test heading size 4', function (): void {
    $value = heading('RAD')->size4()->renderToString();
    expect(str_replace([PHP_EOL, " "], "", $value))->toBe("\e[0m\e[1mRAD\e[22m");
});

test('test heading size 5', function (): void {
    $value = heading('RAD')->size5()->renderToString();
    expect(str_replace([PHP_EOL, " "], "", $value))->toBe("\e[0m\e[2mRAD\e[22m");
});
